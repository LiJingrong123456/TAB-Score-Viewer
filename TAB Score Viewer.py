...
Last Modified: 2026-07-24 (v2.5.3 - 启动时音色库缺失提示)
最后修改: 2026-07-24 (v2.5.3 - 启动时音色库缺失提示)
...
class DisplayWindow(QMainWindow):
    ...
    def _create_control_panel(self)->QWidget:
        ...
        # 快捷键帮助
        # 使用实例变量以便在 GTP 模式下切换文本, 隐藏「调整位置」「微调速度」两项
        # GTP 模式始终显示快捷键区, 仅替换 help_txt 文本为 shortcut_help_gtp
        ...
        self.help_txt.setTextInteractionFlags(Qt.TextSelectableByMouse)  # 允许用户复制快捷键文本
    ...
    def _on_content_loaded(self, images=None)->None:
        ...
        if self.file_type == 'gtp':
            ...
            # GTP模式: 快捷键区保留显示, 但隐藏「调整位置」「微调速度」两项 (用户要求)
            # 原因: GTP 模式由 BPM 驱动播放, 不需要手动调整位置/微调速度
            if hasattr(self, 'shortcut_group_box'):
                self.shortcut_group_box.setVisible(True)
            if hasattr(self, 'help_txt'):
                self.help_txt.setText(I18n.t("control_panel.shortcut_help_gtp"))
        else:
            ...
            # 非GTP模式: 显示完整快捷键帮助 (包含「调整位置」「微调速度」)
            if hasattr(self, 'shortcut_group_box'):
                self.shortcut_group_box.setVisible(True)
            if hasattr(self, 'help_txt'):
                self.help_txt.setText(I18n.t("control_panel.shortcut_help"))
...
class SelectionWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # v2.5.3: 音色库缺失提示标志
        self._soundfont_prompted = False

        self.init_ui()
        # 设置 objectName 让 QSS 选择器 QMainWindow#SelectionWindow 精确匹配本窗口
        # 解决子控件(对话框)继承父 QSS 时导致主题样式不立即生效的问题
        self.setObjectName("SelectionWindow")
        self._apply_theme()  # 应用深色主题
        # 监听主题变化信号: 与 DisplayWindow 一致,确保外部或自身触发主题切换时自动刷新
        try:
            ThemeManager.theme_changed.connect(self._on_theme_changed_external)
        except Exception:
            pass
    ...
    def _apply_theme(self)->None:
        ...
        性能优化(P0-1): 使用 _get_cached_qss() 缓存QSS字符串。
        原理: setStyleSheet 会自动将样式传播给所有子控件 (QLabel/QPushButton/QListWidget 等)。
        """
        theme_name = ThemeManager.current_name()
        t = ThemeManager.current()
    ...
    def _on_theme_changed_external(self, _theme_name: str) -> None:
        """
        主题变更信号槽: ThemeManager.theme_changed 触发时立即刷新本窗口样式
        避免遗漏: 当 GTP 渲染主题等触发 theme_changed 时,主窗口也同步刷新
        """
        self._apply_theme()
    ...
    def _resolve_soundfont_search_paths(self) -> list:
        """
        v2.5.3: 返回所有候选音色库目录（仅返回存在的目录）。
        复用 _init_audio_engine (第 3059-3098 行) 中的路径解析逻辑。
        """
        import sys, os
        paths: list = []
        try:
            if getattr(sys, 'frozen', False):
                exe_dir = os.path.dirname(sys.executable)
                # (1) onedir: exe同目录/_internal/soundfont/
                p_internal = os.path.join(exe_dir, '_internal', 'soundfont')
                if os.path.isdir(p_internal):
                    paths.append(p_internal)
                # (2) onedir: exe同目录/soundfont/ (Windows spec 用 '.')
                p_exe = os.path.join(exe_dir, 'soundfont')
                if os.path.isdir(p_exe):
                    paths.append(p_exe)
                # (3) .app bundle: .app/Contents/Resources/soundfont/
                if '.app/Contents/MacOS' in exe_dir:
                    resources_dir = os.path.join(os.path.dirname(exe_dir), 'Resources')
                    p_bundle = os.path.join(resources_dir, 'soundfont')
                    if os.path.isdir(p_bundle):
                        paths.append(p_bundle)
            else:
                # 开发模式: 项目根目录
                p_dev = os.path.join(_APP_BASE_DIR, 'soundfont')
                if os.path.isdir(p_dev):
                    paths.append(p_dev)
        except Exception as e:
            print(f"[SoundFont] 解析搜索路径失败: {e}")
        return paths
    ...
    def _check_and_prompt_soundfont(self) -> None:
        ...
        每个进程仅执行一次（_soundfont_prompted 标志控制）。
        """
...
# ============================================================
# 音色库缺失引导对话框 (SoundFontMissingDialog) - v2.5.3 新增
# ============================================================
